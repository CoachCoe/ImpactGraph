...
vi.mock("@/lib/proof", async () => {
  const actual = await vi.importActual<typeof import("@/lib/proof")>("@/lib/proof");
...
const PROOF = {
  ...
  requirements: [
    { requirement: "EVIDENCE_INTEGRITY", status: "PASS", reason: "Re-hashing matches." },
    { requirement: "INDEPENDENT_VERIFICATION", status: "FAIL", reason: "Nobody has signed." },
  ],
  attestations: [
    { id: "a1", type: "OPERATOR", issuer: "Global Water Initiative", wallet: null, status: "RECORDED", onchain: false, transactionHash: null },
  ],
  onchain: null,
...
describe("public proof page", () => {
  it("leads with the organisation that did the work, not the platform", async () => {
    ...
    render(await ProofPage({ params: Promise.resolve({ id: PROOF.claim.id }) }));
  ...
  it("says what the verification does not prove, not only what it does", async () => {
    readProof.mockResolvedValue(ok(PROOF));
    render(await ProofPage({ params: Promise.resolve({ id: PROOF.claim.id }) }));
    const limits = screen.getByRole("heading", { name: "What it does not" }).closest("div");
    expect(within(limits!).getByText(/actually helped anyone/)).toBeInTheDocument();
  });
  ...
  it("offers an embed that is one line and points at the live badge", async () => {
    ...
    expect(snippet.textContent).toContain(`/claims/${PROOF.claim.id}/badge.svg`);
  ...
  it("does not report an unpublished claim as a missing page", async () => {
    ...
    render(await ProofPage({ params: Promise.resolve({ id: "claim-unpublished" }) }));
  ...
  it("puts the status in the social card, so a cached card cannot claim more", async () => {
    readProof.mockResolvedValue(ok({ ...PROOF, claim: { ...PROOF.claim, status: "CHALLENGED" } }));
    const metadata = await generateMetadata({ params: Promise.resolve({ id: PROOF.claim.id }) });
    expect(String(metadata.title)).toContain("Verification withdrawn");
    expect(String(metadata.openGraph?.title)).toContain("Verification withdrawn");
  });
